...
- The Quarto document `eds213_data_cleaning_assign_Maxwell_Pepperdine.qmd` should run if your repo is cloned locally (5 pts; Reproducibility)
...
It looks like there are also non-numeric values in the `Total_cover` column. 
...
It's possible that the summing of the `Snow_cover`, `Water_cover`, and `Land_cover` columns is incorrect for some observations. We will recompute the `Total_cover` and re-examine the data. 
...
To make sure the `Total_cover` column is correct, we will recompute the `Total_cover` column by summing the `Snow_cover`, `Water_cover`, and `Land_cover` columns, and ignoring any `NA` values. 
...
The steps above helped clean the `Total_cover` column. Let's check to see if there are any remaining issues by filtering for all rows that are not equal to 100 and are not NA. 
