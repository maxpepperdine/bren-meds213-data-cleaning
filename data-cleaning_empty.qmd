...
# file names
datadir_raw <- file.path(here("data","raw"))
...
- write some code to create a list of everything that's not numeric in the `Snow_cover` column
- write an if else statement to print all values that aren't a number between 0-100

```{r}
snowsurvey_csv %>% 
  count(Snow_cover) %>% 
  # filter to keep only the rows that are not numeric
...
```{r}
# recplace all the "." observations with NA
snowsurvey_fixed <- snowsurvey_csv %>%
  mutate(Snow_cover = ifelse(Snow_cover == ".", NA, Snow_cover))

# check to make sure this worked; we should see no "." values
snowsurvey_fixed %>%
  filter(Snow_cover == ".") %>% 
  view()
```
...
We should assign it a value of 0 because the 3 columns should equate to 100.
...
species_long <- species_csv %>%
  pivot_longer(cols = 8:357,
               names_to = "species",
               values_to = "species_count", 
               # b/c some species have a "X" in the count column, specify chr
